...
total_per_cap_10000 = data_full %>%
  mutate(total_manufacturing = total_manufacturing/county_pop, 
         total_production = total_production/county_pop,
         total_services = total_services/county_pop,
         total_stores = total_stores/county_pop,
         total_truck_transport = total_truck_transport/county_pop,
         total_warehousing = total_warehousing/county_pop,
         total_whls = total_whls/county_pop
         )
...
production_processing = production_processing %>% 
  ...
  index_man = case_when(
    total_manufacturing <= quantile(total_manufacturing, 0.25, na.rm = T) ~ 1,
    total_manufacturing <= quantile(total_manufacturing, 0.50, na.rm = T) ~ 2,
    total_manufacturing <= quantile(total_manufacturing, 0.75, na.rm = T) ~ 3,
    total_manufacturing > quantile(total_manufacturing, 0.75, na.rm = T) ~ 4),
  index_prod = case_when(
    total_production <= quantile(total_production, 0.25, na.rm = T) ~ 1,
    total_production <= quantile(total_production, 0.50, na.rm = T) ~ 2,
    total_production <= quantile(total_production, 0.75, na.rm = T) ~ 3,
    total_production > quantile(total_production, 0.75, na.rm = T) ~ 4),
  index_ware = case_when(
    total_warehousing <= quantile(total_warehousing, 0.25, na.rm = T) ~ 1,
    total_warehousing <= quantile(total_warehousing, 0.50, na.rm = T) ~ 2,
    total_warehousing <= quantile(total_warehousing, 0.75, na.rm = T) ~ 3,
    total_warehousing > quantile(total_warehousing, 0.75, na.rm = T) ~ 4),
...
dist_consumption <- total_per_cap_10000 %>%
  select(`fips`, `total_services`:`total_truck_transport`, `total_whls`)
```
...
SNAPRetailers_data <- SNAPRetailers %>%
  mutate(
    ...
    state =='NM' & county_name == 'Dona Ana' ~ 'Doña Ana',
...
dist_cons = dist_cons %>% 
  mutate(index_serv = case_when(
    total_services <= quantile(total_services, 0.25, na.rm = T) ~ 1,
    total_services <= quantile(total_services, 0.50, na.rm = T) ~ 2,
    total_services <= quantile(total_services, 0.75, na.rm = T) ~ 3,
    total_services > quantile(total_services, 0.75, na.rm = T) ~ 4),
  index_store = case_when(
    total_stores <= quantile(total_stores, 0.25, na.rm = T) ~ 1,
    total_stores <= quantile(total_stores, 0.50, na.rm = T) ~ 2,
    total_stores <= quantile(total_stores, 0.75, na.rm = T) ~ 3,
    total_stores > quantile(total_stores, 0.75, na.rm = T) ~ 4),
  index_truck = case_when(
    total_truck_transport <= quantile(total_truck_transport, 0.25, na.rm = T) ~ 1,
    total_truck_transport <= quantile(total_truck_transport, 0.50, na.rm = T) ~ 2,
    total_truck_transport <= quantile(total_truck_transport, 0.75, na.rm = T) ~ 3,
    total_truck_transport > quantile(total_truck_transport, 0.75, na.rm = T) ~ 4),
  index_whls = case_when(
    total_whls <= quantile(total_whls, 0.25, na.rm = T) ~ 1,
    total_whls <= quantile(total_whls, 0.50, na.rm = T) ~ 2,
    total_whls <= quantile(total_whls, 0.75, na.rm = T) ~ 3,
    total_whls > quantile(total_whls, 0.75, na.rm = T) ~ 4),
...
ggdraw() +
  draw_plot(arcmap, 0, 0, 1.1, 1) +
  draw_plot(legend, 0, 0.65, 0.20, 0.20)
```
